<template>
  <ion-page>
    ...
    <ion-content>
      <ion-list>
        <ion-item v-for="user of users" :key="user.studentId">
          <ion-avatar @click="viewProfile">
            <img v-if="user.photoUrl" :src="user.photoUrl" />
            <img v-if="!user.photoUrl" src="@/assets/male.png" />
          </ion-avatar>
          <ion-label :router-link="`/chat/${user.studentNumber}`">
        ...
        if (user && searchTerm) {
          if (
            ...
            user.lastname.toLowerCase().indexOf(searchTerm.toLowerCase()) >
              -1 ||
            user.studentNumber.toLowerCase().indexOf(searchTerm.toLowerCase()) >
              -1
          ) {
            return true;
          }
